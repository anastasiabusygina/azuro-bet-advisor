...
echo -e "\n[1] Загрузка матчей..."
echo "npx cross-env CHAIN=$CHAIN SPORT_NAME=\"$SPORT\" ts-node src/scripts/ts/fetchAndFormatMatches.standalone.ts --t=$TIME_WINDOW"
npx cross-env CHAIN=$CHAIN SPORT_NAME="$SPORT" ts-node src/scripts/ts/fetchAndFormatMatches.standalone.ts --t=$TIME_WINDOW | cat
