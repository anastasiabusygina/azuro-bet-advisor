...
// Типы для параметров
type MapperMode = 'market' | 'selection';

// Разбор аргументов командной строки
const args: string[] = process.argv.slice(2);
let mode: MapperMode | null = null;
let outcomeId: number | null = null;

for (const arg of args) {
  if (arg.startsWith('--mode=')) {
    const modeValue = arg.split('=')[1] as MapperMode;
    mode = modeValue;
...
try {
  const params: DictionaryParams = { outcomeId: outcomeId as number };
  
  if (mode === 'market') {
    // Получаем название рынка ставки (тип ставки)
    console.log(dictionaries.getMarketName(params));
  } else if (mode === 'selection') {
    // Получаем название исхода ставки
    console.log(dictionaries.getSelectionName(params));
  } else {
