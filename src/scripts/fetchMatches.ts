...
// Constants
// Используем process.cwd() вместо __dirname для CommonJS
const PROJECT_ROOT = process.cwd();
const DATA_DIR = path.join(PROJECT_ROOT, 'scripts', 'data', 'matches');
...
function getMarketName(conditionId: string): string {
  try {
    ...
    // Упрощенная карта соответствия ID рынка и его названия
    const marketMap: {[key: string]: string} = {
      '0640393189': 'Total Goals Over/Under',
    ...
    // Логирование ошибки
    console.error(`Ошибка при определении типа рынка: ${error instanceof Error ? error.message : String(error)}`);
